...
Custom agenda prefix function displays the parent project name for tasks that belong to a project, and shows nothing for standalone tasks (instead of showing the filename).
#+begin_src emacs-lisp
