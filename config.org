...
#+end_src
** Org Babel
Registers additional languages for org-babel code block evaluation. D2 is used for diagrams rendered to SVG inline.
#+begin_src emacs-lisp
(after! org
...
Native compilation and garbage collection tuning for improved performance, particularly important for LSP and large codebases. The native compilation path fix is required for Apple Silicon Macs to locate GCC libraries. GC threshold is increased to reduce pause frequency during intensive operations.
